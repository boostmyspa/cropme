  ...
  function createViewport() {
    ...
    if (options.viewport.border.enable) {
      ...
      if (options.viewport.border.handles.enable) {
        var topRightHandle = this.properties.topRightHandle = this.properties.topRightHandle || document.createElement('div');
        ...
        (function (handle) {
          ...
          function moveHandler(ev) {
            ...
            var oldPosition = viewport.getBoundingClientRect();

            var _ref = ev.touches ? ev.touches[0] : ev,
                clientX = _ref.clientX,
                clientY = _ref.clientY;

            var dx = clientX - oldPosition.right;
            var dy = oldPosition.top - clientY;

            if (options.viewport.border.handles.preserveAspectRatio) {
              var ratio = options.viewport.width / options.viewport.height;
              var newWidth = Math.min(oldPosition.width + dx, containerPosition.width);
